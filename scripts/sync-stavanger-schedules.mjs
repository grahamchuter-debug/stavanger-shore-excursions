/**
 * Sync Stavanger schedule subset from Norway authority master JSON.
 * SOURCE: ../norway-shore-excursions/src/data/cruise-schedules.generated.json
 * FILTER: port === "stavanger"
 * OUTPUT: src/data/stavanger-cruise-schedules.generated.json
 */
...
const outPath = path.join(root, "src/data/stavanger-cruise-schedules.generated.json");

const payload = JSON.parse(readFileSync(sourcePath, "utf8"));
const rows = (payload.rows || []).filter((r) => r.port === "stavanger");

if (rows.some((r) => String(r.arrival_date || "").startsWith("2028"))) {
  console.error("ABORT: 2028 rows found in Stavanger filter");
  process.exit(1);
}
...
console.log("Synced Stavanger schedules");
