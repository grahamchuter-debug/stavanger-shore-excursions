import schedulePayload from "@/data/stavanger-cruise-schedules.generated.json";

export type StavangerScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
export type StavangerScheduleEntry = {
  date: string;
  ship: string;
  arrival: string;
  departure: string;
...
type SchedulePayload = {
  ...
  rows: StavangerScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const stavangerScheduleIntegrity = payload.integrity;
export const stavangerScheduleMeta = {
  port: payload.port,
...
function toEntry(row: StavangerScheduleRow): StavangerScheduleEntry {
  return {
...
export function getAllStavangerRows(): StavangerScheduleRow[] {
  return payload.rows;
}

export function getStavangerEntries(): StavangerScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getStavangerEntriesForMonthKey(monthKey: string): StavangerScheduleEntry[] {
  return payload.rows
...
export function getStavangerMonthKeysWithCalls(): string[] {
  return [...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7)))].sort();
}

export function getStavangerMonthSummaries() {
